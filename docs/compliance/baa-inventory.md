...
| 3 | **Hosting / cloud provider** (not yet chosen) | Everything — it stores and runs the database (PHI at rest) and the app. | App is host-agnostic and secret-free (ADR-0018); assumes provider-level disk encryption + TLS. | ☐ **[CE]** — **prerequisite before any real PHI.** No provider selected yet; its BAA must be signed first. |
...
- [ ] Select a hosting/cloud provider and execute its BAA (blocks real PHI).
