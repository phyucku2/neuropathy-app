...
<<<<<<< HEAD
| 2 | Store-readiness polish (ADR-0032) — in-app **About + Privacy** screens (auth-less `/about` `/privacy`, reachable by a store reviewer + logged-out user; non-diagnostic disclaimer, build-time version, IP/licensee line, in-app privacy summary pointing to the counsel-reviewed full policy) + **route-level code-splitting** (`React.lazy`/`Suspense` per route + recharts/react vendor `manualChunks`) | ✅ | ✅ | ⏳ | New `frontend/src/features/about/**` (exclusive surface); links added to the LoginPage footer + About/Privacy cross-links (Settings "About & privacy" row DEFERRED — sibling surface). Bundle: single 615 KB entry (gzip 180 KB) → 15.70 KB entry (gzip 5.6 KB); two-bucket vendor split (`charts` recharts/d3 299 KB + `vendor` React/router 268 KB), largest chunk now 299 KB; 500 KB warning gone (a first three-bucket split hit a circular chunk that broke React at runtime — caught by the E2E gate, fixed to two buckets). 230 unit tests ≥90% all four (about feature 100%); 36 Playwright specs incl. new `about-privacy.spec.ts` (auth-less, zero console errors, exercises lazy+Suspense in the browser); tsc/eslint/prettier clean; `cap sync` warning-free; backend untouched; secret scan clean. |
=======
| 2 | Patient data export — "Download my data" (ADR-0031) — right-of-access sibling to account deletion: `GET /me/export` (patient-only, one PHI-free `export_account` audit event) returns the COMPLETE record (account/patient rows, all observations w/ provenance, trajectory snapshot, capability states, clinic + EMR connection metadata) as a typed `ExportOut` envelope (`exported_at`/`schema_version`/`subject_id`). NO secrets EVER — token-free projections, proven by ABSENCE (payload scanned for the token/`token_ref`/hash fixtures). "Download my data" card ABOVE the danger zone; web = JSON download + observations CSV (pure flattener), native = Filesystem cache write + OS Share sheet (`@capacitor/filesystem` + `@capacitor/share`, Cap-6 line, injectable seam). | ✅ | ✅ | ⏳ | Backend 100% coverage on new code (unit + live-Postgres, no-secrets proven over real vault ciphertext); 238 unit tests ≥90% all four; 33 Playwright specs (new: the card renders + a real Blob download fires in the built bundle) zero console errors; `cap sync` warning-free (2 new MIT plugins registered in gradle); prod audit + license + secret scans clean. On-device Share-sheet UX is `npx cap run android` only. |
>>>>>>> 73d155c (feat(export): patient data export — "Download my data" (ADR-0031))
