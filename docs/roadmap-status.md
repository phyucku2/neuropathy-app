...
| 4 | Play release prep — release signing (git-ignored `keystore.properties` + Play App Signing, unsigned fallback), manual `release-android.yml` AAB workflow (optional signing secrets, keystore shredded), Play runbook + listing pack (Data Safety draft from actual data flows) + privacy-policy scaffold (ADR-0026) | ✅ | ⏳ | ⏳ | Publisher decided 2026-07-14: **the user's LLC org account** (needs the LLC's DUNS at signup — user-side). No credential collected. Known gaps recorded: account/data **deletion flow REQUIRED before store submission** (hard Play rule — new roadmap item below), production backend deployment, HIPAA validation + BAAs before real data. Internal testing possible before those. |
