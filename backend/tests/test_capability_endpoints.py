...
def test_b2c_patient_sees_registry_defaults(client: TestClient) -> None:
    ...
    # until the owner turns them on: `ingest_wearable` (ADR-0035 Phase 1 — health-store data
    # is PHI), `ingest_notes` (ADR-0045 P2 #27 — EMR clinical notes are sensitive free-text),
    # and `log_food` (ADR-0042 — a ranged self-tracking estimate). `ingest_symptoms` is NOT
    # opt-in: pain + numbness are the core of the daily neuropathy instrument and default ON
    # (ADR-0049 — amends ADR-0034's Phase-1 opt-in).
