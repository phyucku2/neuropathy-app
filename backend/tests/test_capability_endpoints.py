...
def test_b2c_patient_sees_registry_defaults(client: TestClient) -> None:
    ...
    # is PHI) and `ingest_notes` (ADR-0045 P2 #27 — EMR clinical notes are sensitive free-text).
    # `ingest_symptoms` is NOT opt-in: pain + numbness are the core of the daily neuropathy
    # instrument and default ON (ADR-0049 — amends ADR-0034's Phase-1 opt-in).
    opt_in = {"ingest_wearable", "ingest_notes"}
